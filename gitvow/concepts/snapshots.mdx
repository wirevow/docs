---
title: "Snapshots: the code as the agent left it"
description: "A commit records what was accepted. A snapshot records what the agent produced, edit by edit, whether or not anything was committed afterwards"
---
...
- Ignored files are excluded, and so is the built-in credential-store path list (`.env*`, `.npmrc`, `.netrc`, `.kube/config`, SSH private keys, `*.pem`, `*.key` and the rest; see [trust](/gitvow/trust#credential-store-paths)) plus gitvow's own `.gitvow/` and `.claude/` directories, so a snapshot never contains more than a careful `git add -A` would. `snapshots.exclude` in the policy adds to that list and cannot remove from it. Excluded paths that are already committed stay as they are in HEAD.
