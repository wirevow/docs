---
title: "Decisions: who agreed, recorded"
description: "The gate asks a person before an agent does something consequential. Until 0.12 the record kept only that the question was asked"
---
...
An at-commit finding is not live until it is committed, so the agent keeps working and the finding accumulates silently in the session state. One question per commit replaces several per session. Rules choose their class with `"when": "immediate"` or `"when": "commit"`; Bash rules default to immediate, path and provider rules to commit. See [Policy schema](/gitvow/reference/policy).
...
Prefer merge commits or rebase where the evidence, not only the decision, should reach history. For repositories that squash, the [pull request check](/gitvow/guides/pull-requests) writes a decisions summary into the pull request description so the squash commit inherits it.
