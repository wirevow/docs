...
The knowledge that matters most is the kind everyone already observes, so nobody ever writes it down and no gate ever fires on it. Passive capture finds it in what people say. Ratification is what turns "someone said this once" into "the owner confirmed this is how the system works", and the difference between those two sentences is the whole reason to keep the record in git rather than in a wiki. See [Decisions](/gitvow/concepts/decisions) for the primitive this reuses.
