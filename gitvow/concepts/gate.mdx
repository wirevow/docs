---
title: "The gate"
description: "Before your agent runs a tool, it asks gitvow. gitvow answers in one of three ways. The same policy and the same answers apply to every supported agent; only the way a refusal is delivered differs"
---
...
- **Providers**: optional programs that answer factual questions about the edit, such as whether a file is gate-bearing or a new route would be exposed without authorization. See [Providers](/gitvow/concepts/providers).
...
It inspects tool calls the agent makes through its hooks. A Bash command is inspected as text; a process it spawns is not seen separately. Path checks use the path, not the diff. Regular expressions are a floor; providers and the classifier are how you raise it. See [Write a policy](/gitvow/guides/policy).
