...
description: "Every adapter produces the Claude Code shape documented in Hook payloads: sessionid, transcriptpath, cwd, hookeventname, toolname, toolinput"
---
...
## Normalised payload
Every adapter produces the Claude Code shape documented in [Hook payloads](/gitvow/reference/hooks): `session_id`, `transcript_path`, `cwd`, `hook_event_name`, `tool_name`, `tool_input`. Session ids come from `session_id` (Codex, Gemini, Factory), `conversation_id` (Cursor) or `sessionId` (Copilot, whose payloads are camelCase: `toolName`, `toolArgs`).
