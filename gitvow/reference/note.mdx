---
title: "Session note schema"
description: "An object keyed by the name of another checkout the agent edited during the session, each with count and up to 20 paths relative to that checkout"
---
...
| `usage` | object | tokens by kind, models seen, `estimated_cost_usd`, pricing table used; see [What each session cost](/gitvow/guides/cost) |
...
Empty when every edit stayed inside this repository. The gate evaluated those edits; this repository's note is the only record of them.
