---
title: "Let the agent ask the record"
description: "Everything gitvow records is meant to be consulted by the next session, not only by a reviewer"
---

Four commands answer the questions an agent most often needs answered, and four matching **skills** teach Claude Code, Codex, Cursor and Gemini CLI to call them.
...
All four are read-only, local, and need nothing but git and the files gitvow already writes. An agent that runs no hooks and loads no skill can ask the same questions over MCP through the [record server](/gitvow/guides/record-server).
