---
title: "Install per user or per repo"
description: "With no --agent, install looks for each agent by its configuration directory, its application path and its command, configures every one it finds, and prints"
---
...
- Whatever the agent still needs by hand, printed at the end. Codex CLI has two such steps; see [Use gitvow with other agents](/gitvow/guides/other-agents).
...
install. See [What gitvow reads, writes, executes and sends](/gitvow/trust).
